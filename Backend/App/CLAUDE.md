...
**`RsData<T>`**는 `global/rsData/RsData`에 있다. `record RsData<T>(String resultCode, String message, T data)` 형태이며, 신규 API 응답은 이걸로 감싼다.
...
`OrderControllerTest`에는 **`@BeforeEach`가 두 개** 있었다. `setup()`과 `setUp()`으로 이름만 대소문자가 달랐고, Phase 0에서 하나로 합쳤다. JUnit 5는 둘 다 실행하고 순서를 보장하지 않아 매 테스트마다 메뉴가 중복 생성되고 있었다.
