...
| # | PR | 건드리는 파일 | 선행 |
| --- | --- | --- | --- |
| 1 | `feat: Owner 엔티티와 점주 계정 시드 추가` | 신설 `owner/entity/Owner`, `owner/repository/OwnerRepository`, `global/initData/BaseInitData` | 없음 |
| 2 | `feat: JWT 토큰 발급과 검증 컴포넌트 추가` | 신설 `auth/jwt/JwtTokenProvider`, `auth/OwnerPrincipal`, `build.gradle.kts`, `application.yml`, `.env.example` | 1 |
...
| 6 | `feat: 대기번호로 주문 상태를 조회하는 API 추가` | `OrderController`, `OrderService`, `OrderRepository` | 없음 |
