...
| # | PR | 건드리는 파일 | 선행 |
| --- | --- | --- | --- |
| 1 | `refactor: 컨트롤러의 @Transactional 제거하고 트랜잭션 경계를 서비스로 내린다` | `OrderController`, `MenuController` | — |
| 2 | `feat: Stock에 재고 증감 메서드 추가` | `stock/entity/Stock`, 신설 `stock/exception/OutOfStockException`, 신설 `stock/entity/StockTest` | — |
