...
| 액터 | 인증 | 화면 |
| --- | --- | --- |
| **손님** | 익명, 인증 없음 | `/` 키오스크, `/order/{orderNumber}` 주문 조회 |
...
**점선은 UML 관계다.** `«include»`는 항상 포함되는 필수 하위 흐름, `«extend»`는 특정 조건에서만 끼어드는 확장이다. UC-10이 UC-02의 확장인 것이 이 다이어그램의 핵심이다. **동시성은 별개 기능이 아니라 주문 흐름이 경쟁 조건에 놓였을 때의 얼굴**이다.
...
| [UC-07](#uc-07-메뉴를-등록-수정-삭제한다) | 메뉴를 등록, 수정, 삭제한다 | 점주 | 필수 |
...
| 4 | 손님 | 메뉴를 골라 수량과 함께 장바구니에 담는다 | 없음 |
| 5 | 시스템 | 장바구니 소계와 총액을 즉시 갱신한다 | 없음 |
...
| **관련 요구사항** | FR-KSK-01, FR-KSK-02, FR-KSK-03, FR-KSK-04, FR-KSK-07, FR-ORD-05, FR-ORD-06, FR-ORD-07, FR-ORD-10, FR-ORD-12, FR-ORD-14, FR-STK-02, FR-STK-03, NFR-DATA-02, NFR-DATA-03 |
...
| 5 | 시스템 | 이메일로 주문 주체를 찾고, 없으면 만든다. 회원가입이 아니라 익명 주문 주체다 | 없음 |
| 6 | 시스템 | 주문을 `CONFIRMED` 상태로 만든다. 결제까지 끝난 상태라는 뜻이다 | FR-ORD-01 |
| 7 | 시스템 | **`menuId` 오름차순으로 정렬한 뒤** 아이템마다 재고를 차감한다. 정렬은 데드락 회피용이다 | FR-STK-02 |
| 8 | 시스템 | 아이템마다 **그 시점의 메뉴 가격과 이름을 스냅샷**해 붙인다. 나중에 메뉴가 바뀌거나 판매중지돼도 이 주문의 금액과 표시는 안 변한다 | FR-ORD-05, FR-ORD-14 |
...
| 14 | 손님 | 대기번호를 확인하고 카운터에서 기다린다 | 없음 |
...
| **실패 사후조건** | 없음 |
| **관련 요구사항** | FR-KSK-06, FR-KSK-09, FR-KSK-11, FR-ORD-08, FR-ORD-09, FR-ORD-14, FR-MNU-06, NFR-UX-04 |
| **인수 기준** | AC-01, AC-13 |
...
| 5 | 손님 | 상태를 확인한다. 접수됨, 제조중, 준비완료 중 하나 | 없음 |
...
| # | 조건 | 결과 | 근거 |
| --- | --- | --- | --- |
| E1 | 없는 대기번호를 넣었다 | **404** | §9-5 |
| E2 | 남의 이메일로 주문 내역을 통째로 보려 한다 | **그런 경로가 존재하지 않는다** | FR-KSK-09 / AC-13 |
| E3 | 남의 대기번호를 찍어 넣었다 | **그 주문의 상태와 금액이 보인다.** 막지 않기로 한 것이고, 대신 응답에 이메일 같은 개인 식별정보가 없다 | FR-KSK-11 / §10 |
...
>
> **E3이 그 판단의 뒷면이다.** 같은 사실에서 조회는 열어두고 취소만 막았다. 번호를 아는 사람이 남의 주문을 **들여다보는 것**과 **무르는 것**은 무게가 다르고, 카운터에서 번호를 소리내어 부르는 이상 번호 자체는 이미 공개 정보다. 대신 그 응답에 개인 식별정보를 섞지 않는 선을 요구사항으로 못박았다. FR-KSK-11.
...
| # | 조건 | 결과 | 근거 |
| --- | --- | --- | --- |
| E1 | 비밀번호가 틀렸다 | 인증 실패. **어느 쪽이 틀렸는지 알려주지 않는다** | 없음 |
| E2 | 토큰 없이 점주 API를 부른다 | **401.** `GET /api/orders`, 상태 변경, 메뉴 쓰기, 재고 조정, 이미지 업로드 전부 | FR-AUTH-04 / AC-12 |
| E3 | 서명이 위조됐거나 형식이 깨졌다 | **401.** 검증 실패는 전부 같은 결과다 | 없음 |
...
| 3 | 시스템 | 주문 목록을 **주문 시각 오름차순(FIFO)**으로 내려준다. 먼저 들어온 것을 먼저 만든다. 화면은 그중 **아직 끝나지 않은 주문만** 남긴다. 거르는 주체는 화면이고 API는 UC-11과 같은 것을 쓴다 | FR-KIT-01, FR-KIT-02 |
...
| 8 | 손님 | 대기번호가 불리면 카운터에서 받아간다 → [UC-03](#uc-03-대기번호로-자기-주문-상태를-확인한다) | 없음 |
...
| E3 | 존재하지 않는 주문을 넘기려 한다 | **400** | 없음 |
| E4 | 상태 값 자체가 잘못됐다 | **400** | 없음 |
...
| **실패 사후조건** | 메뉴가 바뀌지 않는다 |
| **관련 요구사항** | FR-MNU-01 ~ FR-MNU-09, FR-ORD-14, FR-ADM-01, FR-ADM-03, FR-ADM-04, FR-AUTH-05, NFR-SEC-06 |
| **인수 기준** | AC-01 |
...
| 3 | 점주 | 이름, 카테고리, 가격을 넣고, 필요하면 이미지를 올린다 → [UC-08](#uc-08-메뉴-이미지를-업로드한다) | 없음 |
...
| # | 조건 | 결과 |
| --- | --- | --- |
| A1 | 가격이나 이름을 수정한다 | **과거 주문 내역은 변하지 않는다.** 새 주문부터 새 값이다. FR-MNU-06, FR-ORD-14 / AC-01 |
| A2 | 메뉴를 삭제한다 | **판매가 멈출 뿐 행은 남는다.** 손님 목록에서 사라지고, 그 메뉴가 담긴 과거 주문과 재고 행과 이미지 파일은 유지된다. FR-MNU-09, FR-FILE-08 |
| A4 | 판매중지한 메뉴를 다시 팔고 싶다 | 되살리는 기능은 없다. **새로 등록한다.** `REQUIREMENTS.md §10` |
...
| E3 | 없는 메뉴를 수정하거나 삭제한다 | **404.** 다만 이미 판매중지된 메뉴를 또 삭제하면 **멱등하게 성공한다.** 두 탭에서 같은 버튼을 누르는 일을 오류로 만들지 않는다 | FR-MNU-05 / §9-5 |
...
> `«include»`. [UC-07](#uc-07-메뉴를-등록-수정-삭제한다)의 3단계에서 호출된다.
...
| 2 | 점주 | **얼마나 늘리거나 줄일지**를 넣는다. 최종 수량이 아니라 바꿀 양이다 | FR-STK-08 |
...
| # | 조건 | 결과 | 근거 |
| --- | --- | --- | --- |
| E1 | 결과가 음수가 되는 조정 | **409.** 재고는 어떤 경로로도 음수가 되지 않는다 | FR-STK-06, FR-STK-08, NFR-DATA-04 |
...
>
> **2단계가 최종 수량이 아니라 바꿀 양인 이유가 거기 있다.** 실사 결과를 그대로 받으려면 수량을 대입하는 세 번째 문이 필요해지고, 그 문만 음수 검사를 따로 해야 한다. 점주 화면이 실사 수량을 다루고 싶다면 **화면이 차이를 계산해서 보낸다.** `REQUIREMENTS.md §5-4`.
...
**락 전략은 셋이다. 세 가지 각각에서 위 흐름이 성립해야 한다**
...
sequenceDiagram
    ...
    rect rgb(255, 240, 240)
        Note over SYS,ST: ① 락이 없다면. 커밋 히스토리에 남은 관측이다
        SYS->>ST: A가 읽는다 → 1
        SYS->>ST: B가 읽는다 → 1
        SYS->>ST: A가 쓴다 → 0
        SYS->>ST: B가 쓴다 → -1
    ...
    rect rgb(240, 250, 240)
        Note over SYS,L: ② 락이 있다면. 완성된 동작이다
        SYS->>L: A가 락을 잡는다
        SYS->>ST: 읽고 → 1, 차감 → 0, 커밋
        L-->>SYS: A가 락을 놓는다
        SYS->>L: B가 락을 잡는다
...
| --- | --- | --- | --- |
| UC-01 | FR-MNU-01, FR-KSK-05, FR-KSK-08, FR-KSK-10, FR-ORD-13, FR-STK-07 | NFR-UX-02 | 없음 |
| **UC-02** ★ | FR-KSK-01,02,03,04,07, FR-ORD-05,06,07,10,12,14, FR-STK-02,03 | NFR-DATA-02, NFR-DATA-03, NFR-UX-03 | AC-02, AC-04, AC-05, AC-06 |
| UC-03 | FR-KSK-06, FR-KSK-09, FR-KSK-11, FR-ORD-08, FR-ORD-09, FR-ORD-14, FR-MNU-06 | NFR-UX-04 | AC-01, AC-13 |
...
| UC-07 | FR-MNU-01 ~ 09, FR-ADM-01,03,04, FR-AUTH-05, FR-ORD-14 | NFR-SEC-06 | AC-01 |
| UC-08 | FR-FILE-01 ~ 08 | NFR-OPS-05 | 없음 |
| UC-09 | FR-STK-01,04,06,07,08,09, FR-ADM-02 | NFR-DATA-04 | 없음 |
| **UC-10** ★★ | FR-STK-06 | NFR-CON-01 ~ 07, NFR-DATA-04, NFR-TEST-03, NFR-TEST-04 | **AC-09**, AC-10, AC-11 |
| UC-11 | FR-ADM-05, FR-KIT-02,03, FR-ORD-08,09, FR-KSK-09 | 없음 | AC-12, AC-13 |

**인수 기준은 전부 어느 유스케이스에든 걸린다.** 걸리지 않는 AC가 생긴다면 그건 유스케이스가 빠졌다는 뜻이다.
...
| FR-MNU-07, FR-ORD-15 | API 경로와 응답 포맷의 통일. 손님도 점주도 이걸 경험하지 않는다. **BFF 핸들러는 경험한다.** 응답 껍데기가 바뀌면 그쪽을 같이 고쳐야 한다 |
