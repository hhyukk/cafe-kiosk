...
deleted_at은 소프트 삭제 컬럼이다. 근거는 `menu/entity/Menu.java`의 deletedAt과 discontinue다. 메뉴를 하드 삭제하면 order_item과 stock이 그 행을 참조하고 있어 외래키 위반으로 삭제가 실패한다. 팔린 적 있는 메뉴는 영원히 지울 수 없다는 뜻이다. 그렇다고 참조를 끊고 지우면 과거 주문이 무엇을 팔았는지 알 수 없게 된다. 그래서 삭제를 행을 없애는 일이 아니라 판매를 중단하는 일로 다시 정의했다. 손님 화면의 메뉴 목록은 `deleted_at is null`인 행만 보고, 과거 주문은 삭제된 메뉴를 그대로 참조한다.
...
필터를 거는 자리는 엔티티가 아니라 리포지토리다. `MenuRepository.findAllByDeletedAtIsNull`과 `findByIdAndDeletedAtIsNull`이 그것이고, Hibernate의 `@SQLRestriction`은 쓰지 않는다. 엔티티에 필터를 붙이면 그 엔티티를 읽는 모든 경로에 걸리므로 판매 중단된 메뉴를 참조하는 `order_item`의 연관이 초기화되지 않는다. 참조를 살려 두는 것이 소프트 삭제의 목적인데 그 목적을 정면으로 깬다. 대신 `JpaRepository`가 주는 `findById`와 `findAll`은 필터 없이 남겨, 판매 중단 여부와 무관하게 행 자체를 봐야 하는 자리가 쓴다.
...
| quantity | int | NOT NULL, CHECK quantity >= 0 | 현재 재고 수량. 새 메뉴는 0으로 시작한다 |
...
근거는 `MenuService.createMenu`다. 초기 수량은 0이므로 새 메뉴는 품절 상태로 태어나고, 팔 수 있게 만드는 것은 재고 조정이 따로 한다. 그 판단은 `docs/ADR/ADR-0004`에 있다. 이 두 줄이 붙기 전까지는 `POST /api/menu`로 등록한 메뉴에 재고 행이 없었고, ERD가 있으면 안 된다고 적은 상태를 코드가 정상적으로 만들어 내고 있었다.
...
version이 낙관적 락 컬럼이다. 근거는 `stock/entity/Stock.java`의 version이다. 락 세 전략 중 낙관적 락에서만 쓰이지만 컬럼은 항상 존재해도 무해하다. `@Version`이 붙으면 모든 UPDATE에 version 조건이 따라붙는데, 비관적 락은 `SELECT FOR UPDATE`로 행 접근 자체를 직렬화하므로 버전 충돌이 애초에 일어나지 않는다. 분산 락도 마찬가지로 임계 구역이 하나뿐이라 충돌하지 않는다. 그래서 세 전략을 같은 스키마 위에서 갈아 끼우며 비교할 수 있다. 전략마다 스키마를 바꿔야 한다면 비교 자체가 공정하지 않다.
...
이름 스냅샷은 나중에 붙었다. 한때 주문 내역을 보여줄 때 살아 있는 menu 행에서 이름을 읽었다. `OrderService.toItemDTO`가 `orderItem.getMenu().getMenuName()`을 호출했고, 가격은 굳혀 놓고 이름은 그러지 않았으니 절반만 스냅샷인 셈이었다. 메뉴 이름을 고치면 과거 주문의 품목명이 함께 바뀌었다. 지금은 `OrderItem` 생성자가 가격과 함께 이름도 복사하고, `toItemDTO`는 `orderItem`만 읽는다. FR-ORD-11.

두 스냅샷을 모두 생성자가 복사하는 것이 규칙이다. 호출자에게 맡기면 언젠가 스냅샷을 빠뜨리는 경로가 생긴다.
...
실제로는 조인도 쓰지 않았다. `MenuService.findAllWithStock`이 메뉴 목록과 재고를 각각 한 번씩 읽어 메모리에서 menuId로 맞춘다. 메뉴가 몇 개든 쿼리는 두 방이고, 엔티티가 서로를 들고 있지 않은 것은 물론 SQL 조인도 만들지 않으므로 메뉴를 읽는 다른 경로에 재고가 따라 올라올 여지가 없다. 재고 행이 없는 메뉴는 그 맞춤에서 빈자리가 되고, 응답에 수량 0이 아니라 null로 실린다. 없는 것과 0개인 것은 다른 사실이다.
...
락 세 전략을 차례로 붙이고 같은 동시성 테스트에 건다. NFR-CON-03. 비관적 락과 낙관적 락이 붙었고 분산 락이 남았다. 각 전략이 스키마에 무엇을 요구하는지는 서로 다르다.
