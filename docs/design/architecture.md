...
시드 이미지를 앞쪽에 두면 안 되는 이유는 이렇다. 앞쪽은 gitignore되어 있어 **clone한 사람에게 파일이 전달되지 않는다.**
...
| 시드 | `BaseInitData`가 `dev`에서만 심는다. 메뉴 3개 + 재고 **100 / 50 / 3** + 점주 계정 1개 |
...
frontend/src/app
├── page.tsx                      /            키오스크, 손님
├── order/[orderNumber]/page.tsx  /order/{n}   주문 완료와 조회, 손님
...
├── auth/AuthIntegrationTest.java            ← 로그인 성공과 실패, 토큰 없이 401
